...
*Note: As shown with CloudCompare Interpolate, the black areas represent *nodata* and are not **real elevation data**. When the resulting raster is read by software such as GDAL or QGIS, these *nodata* areas may be interpreted as a value of 0, causing the reported minimum elevation to be incorrect. This means the height range used to create the heightmap will not be accurate. It is possible to replace the *nodata* areas with the **true minimum elevation**, but this out of the scope of this tutorial and is not covered in this tutorial.
